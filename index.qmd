...
# The <red>Problem</red> of <green>Shape Reconstruction</green> {background-image="drawing.png" background-opacity="0.3" .smaller}
...
## The <green>Theme</green> of <red>the Research</red>
...
> ...fll in the small triangles of edges in the graph $X_\varepsilon^{1}$ by actual $2$-dimensional triangles to obtain $X_\varepsilon^{2}\subset X$, then $\pi_1(X_\varepsilon^{2})=\pi_1(X)$ for $\varepsilon,\delta_\varepsilon\to0$ with $\varepsilon/\delta_\varepsilon\to\infty$.
...
## Bounded <green>Curvature</green> Spaces {.smaller}
...
- $\mathrm{CAT}(\kappa)$: geodesic triangles are "slimmer" than corresponding "model triangles" in a standard space of constant curvature $\kappa$.
...
- For any two sample points $a,b\in S$, define $d_S^\varepsilon(a,b)$ to be the shortest distance when hopping over the points of $S$ with a step size of $\varepsilon$.
:::
...
## Large-scale Distortion {.smaller}
...
- <green>Large-scale distortion</green>: $$\delta^R_\beta(X)=\sup_{d^L(a,b)\geq R}\frac{d^L_X(a,b)}{d^L_{X^\varepsilon}(a,b)}$$
- $\delta^\varepsilon_\beta(X)\to1$ as $\varepsilon\to0$, provided $X$ is compact
:::
...
# <green>Geometric</green> Reconstruction 
## Back to Gromov: <green>Geometric Shadow</green> {.smaller}

- Vietoris--Rips complexes are abstact, hence contain only topological information
- **<green>Geometric reconstruction</green>**: entails constructing $\widetilde{X}\subset\mathbb R^N$ from samples so that $\widetilde{X}\simeq X$ and <green>$d_H(\widetilde{X}, X)$ is controlled</green>.
- A good candidate for $\widetilde{X}$ is the *shadow* of a topologically-faithful Vietoris--Rips.
...
If $S\subset \mathbb R^2$ is compact and $d_H(G,S)<\tfrac{1}{2}\xi\varepsilon$, then the shadow $sh(\mathcal R_\beta^\varepsilon(S))$ is homotopy equivalent to $G$. Moreover, $d_H(sh(\mathcal R_\beta^\varepsilon(S)),G)<\left(\beta+\frac{1}{2}\xi\varepsilon\right)$. 
:::

# Future Directions {background-image="drawing.png" background-opacity="0.3"}

- Geometric reconstruction of submanifolds via Rips shadow (*in preparation*)
